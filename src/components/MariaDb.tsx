...
import { MariaDbRelease } from '../types';
...
export function MariaDb({ activeVersion, onVersionChange }: MariaDbProps = {}) {
  const [releases, setReleases] = useState<MariaDbRelease[]>([]);
  ...
  const { progress, clearProgress } = useDownloadProgress('mariadb');
  ...
  const fetchData = async () => {
    ...
    try {
      const [installed, available] = await Promise.all([
        ...
        fetchWithCache('cache_mariadb_versions', () => invoke<MariaDbRelease[]>('fetch_mariadb_versions').catch(() => [])),
      ]);
      setInstalledVersions(installed);
      const filtered = filterHighestPatches(available, installed);
      setReleases(filtered);
    } catch (e) {
      toast.error('Failed to load MariaDB versions');
    } finally {
      setIsFetching(false);
    }
  };
  ...
  const checkInitialized = async () => {
    try {
      const initialized = await invoke<boolean>('is_mariadb_initialized');
  ...
  const checkPathStatus = async (version: string) => {
    try {
      const path = `C:\\kythia\\bin\\mariadb\\${version}\\bin`;
  ...
  const togglePath = async (checked: boolean) => {
    ...
    try {
      if (checked) {
        await invoke('add_to_path', { service: 'mariadb', exactPath: path });
        toast.success(`Added MariaDB ${version} to System PATH. You may need to restart your terminal.`);
      } else {
        await invoke('remove_from_path', { service: 'mariadb' });
        toast.success(`Removed MariaDB from System PATH.`);
      }
  ...
  const handleInstall = async () => {
    ...
    clearProgress();
    try {
      const msg = await invoke<string>('install_mariadb', { 
  ...
  const handleInitialize = async () => {
    ...
    try {
      const msg = await invoke<string>('init_mariadb', { version: installedVersions[0] });
      toast.success(msg);
      checkInitialized();
    } catch (e: any) {
  ...
  return (
    <div className="space-y-12 max-w-2xl pb-10">
      <section>
        <div className="mb-6">
          <h2 className="text-2xl font-semibold tracking-tight">Install MariaDB</h2>
          <p className="text-sm text-muted-foreground mt-1">Select a version from mariadb.org to install.</p>
        </div>
          ...
          <div className="flex gap-4 items-end">
            <div className="flex-1 space-y-2">
              ...
              <Select 
                ...
                items={releases.map(r => ({ value: r.version, label: `MariaDB ${r.version}` }))}
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  {releases.map(r => (
                    <SelectItem key={r.version} value={r.version} label={`MariaDB ${r.version}`}>MariaDB {r.version}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
                ...
                <div>
                  <div className="flex items-center space-x-4 mb-1">
                    ...
                    <Select 
                      ...
                      items={installedVersions.map(v => ({ value: v, label: `MariaDB v${v}` }))}
                    >
                      ...
                      <SelectContent>
                        {installedVersions.map(v => (
                          <SelectItem key={v} value={v} label={`MariaDB v${v}`}>MariaDB v{v}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  </div>
              ...
              <ul className="space-y-2">
                {installedVersions.map(v => (
                  <li key={v} className="flex items-center justify-between p-3 rounded-md bg-secondary/30 border border-border/50">
                    <span className="font-medium">MariaDB v{v}</span>
