...
import { MysqlRelease } from '../types';
...
export function Mysql({ activeVersion, onVersionChange }: MysqlProps = {}) {
  const [releases, setReleases] = useState<MysqlRelease[]>([]);
  ...
  const { progress, clearProgress } = useDownloadProgress('mysql');
  ...
  const fetchData = async () => {
    ...
    try {
      const [installed, available] = await Promise.all([
        ...
        fetchWithCache('cache_mysql_versions', () => invoke<MysqlRelease[]>('fetch_mysql_versions').catch(() => [])),
      ]);
      setInstalledVersions(installed);
      const filtered = filterHighestPatches(available, installed);
      setReleases(filtered);
    } catch (e) {
      toast.error('Failed to load MySQL versions');
    } finally {
      setIsFetching(false);
    }
  };
  ...
  const checkInitialized = async () => {
    try {
      const initialized = await invoke<boolean>('is_mysql_initialized');
  ...
  const checkPathStatus = async (version: string) => {
    try {
      const path = `C:\\kythia\\bin\\mysql\\${version}\\bin`;
  ...
  const togglePath = async (checked: boolean) => {
    ...
    try {
      if (checked) {
        await invoke('add_to_path', { service: 'mysql', exactPath: path });
        toast.success(`Added MySQL ${version} to System PATH. You may need to restart your terminal.`);
      } else {
        await invoke('remove_from_path', { service: 'mysql' });
        toast.success(`Removed MySQL from System PATH.`);
      }
  ...
  const handleInstall = async () => {
    ...
    clearProgress();
    try {
      const msg = await invoke<string>('install_mysql', { 
  ...
  const handleInitialize = async () => {
    ...
    try {
      const msg = await invoke<string>('init_mysql', { version: installedVersions[0] });
      toast.success(msg);
      checkInitialized();
    } catch (e: any) {
  ...
  return (
    <div className="space-y-12 max-w-2xl pb-10">
      <section>
        <div className="mb-6">
          <h2 className="text-2xl font-semibold tracking-tight">Install MySQL</h2>
          <p className="text-sm text-muted-foreground mt-1">Select a version of MySQL Community Server to install.</p>
        </div>
              ...
              <ul className="space-y-2">
                {installedVersions.map(v => (
                  <li key={v} className="flex items-center justify-between p-3 rounded-md bg-secondary/30 border border-border/50">
                    <span className="font-medium">MySQL v{v}</span>
